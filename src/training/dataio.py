...
class JointScfTrainDataset(Dataset):
    def __init__(self, sidelength, depth_aug=False, multiview_aug=False, phase='train', obj_class='all', o_dim=5):
        self.o_dim = o_dim
        ...
        bottle_path = osp.join(path_util.get_ndf_data(), 'training_data/bottle_table_all_pose_4_cam_half_occ_full_rand_scale')
        bowl_path = osp.join(path_util.get_ndf_data(), 'training_data/bowl_table_all_pose_4_cam_half_occ_full_rand_scale')
        ...
        # self.scf_minmax = pickle.load(open(osp.join(path_util.get_ndf_data(), 'training_data/scf_minmax.p'), "rb"))
    ...
    def get_item(self, index):
        try:
            ...
            # coord, _, _, scf = self.shapenet_dict[category_id][voxel_path]
            shapenet_dict = {'03797390': 'mug', '02880940': 'bowl', '02876657': 'bottle'}
            scf_data = np.load(osp.join(path_util.get_ndf_data(), f'training_data/scfs/{shapenet_dict[category_id]}/{shapenet_id}.npy'))
            coord = scf_data[:,:3]
            scf = scf_data[:,3:]
...
class JointSdfTrainDataset(Dataset):
    def __init__(self, sidelength, depth_aug=False, multiview_aug=False, phase='train', obj_class='all', scaling=10):
        ...
        self.shapenet_mug_dict = pickle.load(open(osp.join(path_util.get_ndf_data(), 'training_data/sdf_shapenet_mug.p'), 'rb'))
        self.shapenet_bowl_dict = pickle.load(open(osp.join(path_util.get_ndf_data(), 'training_data/sdf_shapenet_bowl.p'), "rb"))
        self.shapenet_bottle_dict = pickle.load(open(osp.join(path_util.get_ndf_data(), 'training_data/sdf_shapenet_bottle.p'), "rb"))
    ...
    def get_item(self, index):
        try:
            ...
            # load in voxel occupancy data
            voxel_path = osp.join(category_id, shapenet_id, 'models', 'model_normalized_128.mat')
            coord, _, _,_, sdf = self.shapenet_dict[category_id][voxel_path]
            ...
            coord = coord + offset
            coord = coord * data['mesh_scale']
            label = label * np.linalg.norm(data['mesh_scale']) * self.scaling
