...
import src.model.vnn_occupancy_net_pointnet_dgcnn as vnn_occupancy_network
...
train_dataset = dataio.JointScfTrainDataset(opt.sidelength, o_dim=opt.o_dim,obj_class=opt.obj_class)
val_dataset = dataio.JointScfTrainDataset(opt.sidelength,o_dim=opt.o_dim,phase='val', obj_class=opt.obj_class)
...
model = vnn_occupancy_network.VNNOccNet(latent_dim=256,o_dim=opt.o_dim,sigmoid=False).cuda()
